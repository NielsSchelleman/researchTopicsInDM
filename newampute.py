...
class MultivariateAmputation(TransformerMixin):
    ...
    def _calculate_sumscores(self, data_group: Matrix, pattern_ind: int) -> ArrayLike:
        ...
        wss = sum(np.multiply(np.array(pd.DataFrame(data_group).fillna(0)), self.weights[pattern_ind, :].T).T)
        return wss
    ...
    def fit_transform(self, X: Matrix) -> Matrix:
        ...
        for pattern_ind in range(self.num_patterns):
            ...
            if self.norm == np.inf:
                ...
                wss = (sum(np.array(wss_stash)**self.norm)**(1/self.norm))/(len(wss_stash)**1/self.norm)
    ...
    def do_ordinal_ampute(self,
                          ord_cols: list,
                          data: pd.DataFrame,
                          orders: list,
                          lproportion_based: list,
                          relations: list,
                          norm: int = 1):
        self.norm = norm
        interval_data = data.drop(ord_cols, axis=1)
        for i in range(len(ord_cols)):
            self.patterns = np.c_[self.patterns, np.ones(np.shape(self.patterns)[0])]
            self.weights = np.c_[self.weights, np.ones(np.shape(self.weights)[0])]
        for column in interval_data.columns:
            interval_data[column] = interval_data[column].astype('float')
        for i in range(len(ord_cols)):
            column_name = 'dummy'+str(i)
        ...
        a = self.fit_transform(X=interval_data)
        rev_maps = {}
        for col in ord_cols:
